    ...
    <div style={{ fontFamily: "'Inter', sans-serif" }} className="bg-gray-50 min-h-screen">
      ...
      <div className="relative bg-[#1a3c22] text-white py-12 md:py-24 overflow-hidden">
        ...
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <h1 className="text-3xl md:text-5xl lg:text-6xl font-extrabold mb-4 md:mb-6 tracking-tight drop-shadow-sm" style={{ fontFamily: "'Montserrat', sans-serif" }}>
            Get in Touch
          </h1>
      ...
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-16">
        {/* Changed to flex-col on mobile to control ordering with order-1 and order-2 */}
        <div className="flex flex-col lg:grid lg:grid-cols-3 gap-8 lg:gap-10 items-start">
          
          {/* ─── MAIN COLUMN: CONTACT FORM (Moves to TOP on mobile) ─── */}
          <div className="w-full lg:col-span-2 space-y-6 md:space-y-8 order-1 lg:order-2">
            <div className="bg-white rounded-3xl border border-gray-100 p-5 md:p-10 shadow-lg shadow-gray-100/50">
              <h2 className="text-2xl md:text-3xl font-extrabold text-gray-900 mb-2" style={{ fontFamily: "'Montserrat', sans-serif" }}>Send Us a Message</h2>
              <p className="text-gray-500 text-sm md:text-base mb-6 md:mb-8">Please select your reason for inquiry to help us route your message correctly.</p>

              {/* Inquiry Type Selection Grid */}
              <div className="mb-8 md:mb-10">
                <label className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-3 block">Select Topic <span className="text-red-500">*</span></label>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                  {inquiryTypes.map((type) => {
                    ...
                    return (
                      ...
                      >
                        <div 
                          className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center transition-colors ${isSelected ? "text-white" : "bg-white text-gray-400 shadow-sm"}`}
              ...
              <div className="min-h-[300px] md:min-h-[400px]">
                {selectedType === "" ? (
                  <div className="text-center py-12 md:py-20 px-4 md:px-6 border-2 border-dashed border-gray-200 bg-gray-50 rounded-3xl animate-in fade-in">
                    ...
                    <h3 className="text-base md:text-lg font-bold text-gray-700 mb-2" style={{ fontFamily: "'Montserrat', sans-serif" }}>Awaiting Selection</h3>
                    <p className="text-gray-500 text-xs md:text-sm max-w-sm mx-auto">Please select a reason for your inquiry from the options above to reveal the correct contact form.</p>
                  </div>
                ) : (
                  <div className="animate-in slide-in-from-bottom-4 fade-in duration-500">
                    
                    {/* Active Selection Banner */}
                    <div className="flex items-start md:items-center gap-3 mb-6 md:mb-8 p-3 md:p-4 rounded-xl border" style={{ backgroundColor: `${selectedTypeData.color}08`, borderColor: `${selectedTypeData.color}20` }}>
                      <CheckCircle size={18} className="mt-0.5 md:mt-0 shrink-0 md:w-[20px] md:h-[20px]" style={{ color: selectedTypeData.color }} />
                    ...
                    {submitted ? (
                      <div className="text-center py-12 md:py-16 px-4 md:px-6 border border-gray-100 rounded-3xl bg-white shadow-sm animate-in zoom-in-95 duration-300">
                        <div className="w-16 h-16 md:w-20 md:h-20 rounded-full flex items-center justify-center mx-auto mb-5 md:mb-6 shadow-md" style={{ backgroundColor: selectedTypeData.color }}>
                          <CheckCircle size={32} className="text-white md:w-[36px] md:h-[36px]" />
                        </div>
                        <h3 className="text-2xl md:text-3xl font-extrabold text-gray-900 mb-2 md:mb-3" style={{ fontFamily: "'Montserrat', sans-serif" }}>Message Sent!</h3>
                        <p className="text-gray-500 mb-6 md:mb-8 max-w-md mx-auto text-sm md:text-base">Thank you for reaching out. Our team has received your inquiry and will respond within 1–2 business days.</p>
                        <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
              {[
                ...
                <div key={i} className="bg-white rounded-3xl border border-gray-100 p-5 md:p-6 text-center flex flex-col items-center justify-center hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group">
                  ...
                  <div className="font-extrabold text-gray-900 text-base md:text-lg mb-1" style={{ fontFamily: "'Montserrat', sans-serif" }}>{stat.title}</div>
          ...
          {/* ─── SIDEBAR: OFFICES (Moves to BOTTOM on mobile) ─── */}
          <div className="w-full lg:col-span-1 space-y-6 order-2 lg:order-1 pt-4 lg:pt-0">
            <h3 className="text-xl md:text-2xl font-extrabold text-gray-900" style={{ fontFamily: "'Montserrat', sans-serif" }}>Our Global Offices</h3>
            
            {/* Office Selection List */}
            <div className="flex overflow-x-auto lg:flex-col pb-4 lg:pb-0 gap-3 lg:gap-4 snap-x hide-scrollbar" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
              {offices.map((office) => (
                <button
                  key={office.id}
                  onClick={() => setSelectedOffice(office)}
                  className={`w-[85vw] sm:w-[300px] lg:w-full shrink-0 snap-center text-left p-3 md:p-4 rounded-2xl border-2 transition-all duration-300 flex items-start gap-3 md:gap-4 group hover:-translate-y-1 hover:shadow-lg active:scale-95 ${
                    selectedOffice.id === office.id 
                      ? "border-[#2A5C32] bg-[#f0f7f1] shadow-md" 
                      : "border-gray-100 hover:border-[#2A5C32]/30 hover:bg-[#f7fbf8] bg-white shadow-sm"
                  }`}
                >
                  <div className={`w-8 h-8 md:w-10 md:h-10 rounded-xl flex items-center justify-center shrink-0 transition-colors duration-300 ${
                  ...
                  <div className="flex-1 min-w-0 mt-0.5">
                    <div className={`text-[9px] md:text-[10px] font-bold uppercase tracking-widest mb-1 transition-colors ${
                      selectedOffice.id === office.id ? "text-[#2A5C32]" : "text-gray-400 group-hover:text-[#2A5C32]/70"
                    }`}>{office.region}</div>
                    <div className="font-extrabold text-gray-900 text-sm md:text-base truncate" style={{ fontFamily: "'Montserrat', sans-serif" }}>{office.city}</div>
                    <div className="text-[11px] md:text-xs font-medium text-gray-500 mt-1 truncate">{office.address}</div>
                  </div>
                  {selectedOffice.id === office.id && (
            ...
            <div className="bg-white rounded-3xl border border-gray-100 p-5 md:p-8 shadow-sm">
              <h4 className="font-extrabold text-gray-900 mb-5 md:mb-6 text-base md:text-lg flex flex-wrap items-center gap-1" style={{ fontFamily: "'Montserrat', sans-serif" }}>
              ...
              <div className="space-y-4 md:space-y-5">
                <div className="flex gap-3 md:gap-4 items-start group">
                  <div className="bg-gray-50 p-2 md:p-2.5 rounded-lg group-hover:bg-[#f0f7f1] group-hover:text-[#2A5C32] transition-colors shrink-0">
                    <MapPin size={16} className="text-gray-400 group-hover:text-[#2A5C32] transition-colors md:w-[18px] md:h-[18px]" />
                  </div>
                  <span className="text-xs md:text-sm font-medium text-gray-600 leading-relaxed mt-0.5 md:mt-1">{selectedOffice.address}</span>
                </div>
                <div className="flex gap-3 md:gap-4 items-center group">
                  ...
                  <a href={`tel:${selectedOffice.phone.replace(/\s+/g, '')}`} className="text-xs md:text-sm font-bold text-gray-600 hover:text-[#2A5C32] transition-colors break-all">{selectedOffice.phone}</a>
                </div>
                <div className="flex gap-3 md:gap-4 items-center group">
                  <div className="bg-gray-50 p-2 md:p-2.5 rounded-lg group-hover:bg-[#f0f7f1] group-hover:text-[#2A5C32] transition-colors shrink-0">
                    <Mail size={16} className="text-gray-400 group-hover:text-[#2A5C32] transition-colors md:w-[18px] md:h-[18px]" />
                  </div>
                  <a href={`mailto:${selectedOffice.email}`} className="text-xs md:text-sm font-bold text-gray-600 hover:text-[#2A5C32] transition-colors break-all">{selectedOffice.email}</a>
                </div>
                <div className="flex gap-3 md:gap-4 items-center group">
                  <div className="bg-gray-50 p-2 md:p-2.5 rounded-lg group-hover:bg-[#f0f7f1] group-hover:text-[#2A5C32] transition-colors shrink-0">
            ...
            {/* Map Frame */}
            <div className="w-full h-48 md:h-56 bg-gray-100 rounded-3xl overflow-hidden border-4 border-white shadow-md relative group">
            ...
            <div className="flex flex-col sm:flex-row gap-3 md:gap-4 pt-2">
              <Link href="/global-presence" className="flex-1 flex items-center justify-between p-3 md:p-4 rounded-2xl border-2 border-transparent bg-white shadow-sm hover:border-[#2A5C32]/20 hover:shadow-md hover:-translate-y-0.5 active:scale-95 transition-all group">
                <div className="flex items-center gap-2 md:gap-3 text-xs md:text-sm font-bold text-gray-800 group-hover:text-[#2A5C32] transition-colors">
                  <Globe size={16} className="text-[#2A5C32] md:w-[18px] md:h-[18px]"/> All Offices
                </div>
              ...
              <Link href="/products" className="flex-1 flex items-center justify-between p-3 md:p-4 rounded-2xl border-2 border-transparent bg-white shadow-sm hover:border-[#2A5C32]/20 hover:shadow-md hover:-translate-y-0.5 active:scale-95 transition-all group">
                <div className="flex items-center gap-2 md:gap-3 text-xs md:text-sm font-bold text-gray-800 group-hover:text-[#2A5C32] transition-colors">
      ...
      <style>{`
        /* Add this to hide scrollbar but keep functionality for the horizontal mobile list */
