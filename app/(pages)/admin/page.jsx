...
const normalizePendingAdmins = (payload) => {
  ...
  const normalizeArray = (items) =>
    Array.isArray(items)
      ? items.map(normalizeAdminItem).filter(
    ...
    <article className="admin-card">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7, color: '#5a7060', fontSize: 12 }}>
          ...
          <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 11.5, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {admin.Email}
          </span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7, color: '#5a7060', fontSize: 12 }}>
...
export default function AdminDashboard() {
  ...
  const stats = useMemo(() => ({
    total:          forms.length,
    newSubmissions: forms.filter((f) => f.status === 'new').length,
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f7f7f5', fontFamily: "'Outfit', sans-serif", color: '#1a2e1e' }}>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .action-btn:hover:not(:disabled) { transform: scale(1.04); }
        ...
        /* Header nav pills — hide name on very small screens */
        .header-name-pill { display: flex; }
        .header-role-pill { display: flex; }
        ...
        @media (max-width: 520px) {
          ...
          .header-role-pill { display: none; }
          .pending-info-grid {
            grid-template-columns: 1fr;
          }
          .db-table { display: none; }
          .mobile-cards { display: flex; flex-direction: column; gap: 12px; padding: 12px; }
      ...
      <header style={{
        ...
        borderBottom: '1px solid rgba(80,140,90,0.25)',
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
            <div style={{
              ...
              background: 'linear-gradient(135deg,#3a8048,#1e5230)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              boxShadow: '0 4px 14px rgba(58,128,72,0.45)',
            }}>
              <Leaf size={17} style={{ color: '#c8f0d0' }} />
            </div>
            <span style={{
              fontSize: 10, fontWeight: 800, letterSpacing: '0.22em', textTransform: 'uppercase',
              color: '#a8c8b0', padding: '4px 10px', borderRadius: 99,
              border: '1px solid rgba(100,160,110,0.3)',
              background: 'rgba(58,128,72,0.15)',
              whiteSpace: 'nowrap',
            }}>
              Admin Console
            </span>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div className="header-name-pill" style={{
            ...
            <div className="header-role-pill" style={{
              ...
              <span>{isSuperAdmin ? 'SUPER ADMIN' : isPendingAdmin ? 'PENDING' : 'LIVE'}</span>
            </div>
            <button
            ...
            >
              <LogOut size={13} /> <span style={{ display: 'none' }} className="logout-text">LOGOUT</span>
              <style>{`@media (min-width: 400px) { .logout-text { display: inline !important; } }`}</style>
            </button>
          </div>
        </div>
      </header>
      ...
      {/* ━━━ MAIN ━━━ */}
      <main style={{ maxWidth: 1440, margin: '0 auto', padding: '24px 16px 60px' }}>
        ...
        {isPendingAdmin ? (
          <section style={{ maxWidth: 680, margin: '0 auto' }}>
            <div style={{
              ...
              boxShadow: '0 16px 50px -24px rgba(15,36,21,0.16)',
              overflow: 'hidden',
            }}>
              <div style={{
                padding: '20px 22px', borderBottom: '1px solid #e4ece4',
                ...
                <div style={{
                  ...
                  <Clock size={19} style={{ color: '#fff' }} />
                </div>
                <div>
                  <h1 style={{ fontSize: 18, fontWeight: 800, color: '#0e1e12', margin: 0 }}>
              ...
              <div style={{ padding: '20px 22px', display: 'flex', flexDirection: 'column', gap: 14 }}>
                <div className="pending-info-grid">
                  <div style={{ borderRadius: 14, border: '1.5px solid #d8e8da', background: '#f6fcf7', padding: '14px 16px' }}>
                  ...
                  <div style={{ borderRadius: 14, border: '1.5px solid #e8d4bc', background: '#fdf6ec', padding: '14px 16px' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
                  <button onClick={fetchForms} disabled={formsLoading} style={{
          ...
          <>
            {/* ── SuperAdmin: Admin Status Management ── */}
            {isSuperAdmin && (
              <section style={{ marginBottom: 24 }}>
                <div style={{
                  background: '#fff', borderRadius: 24,
                  border: '1.5px solid #ccdece',
                  boxShadow: '0 10px 36px -20px rgba(15,36,21,0.12)',
                  overflow: 'hidden',
                }}>
                  {/* Section header */}
                  <div style={{
                    padding: '16px 20px', borderBottom: '1px solid #deeade',
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{
                        ...
                        <Users size={16} style={{ color: '#c0f0cc' }} />
                      </div>
                      <div>
                        <h2 style={{ fontSize: 14, fontWeight: 800, color: '#0e1e12', margin: 0 }}>
                    ...
                    >
                      {pendingAdminsLoading ? <Loader2 size={14} className="animate-spin" /> : <RefreshCcw size={14} />}
                      Refresh List
                    </button>
                  </div>
                  ...
                  <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                    {pendingAdminsNotice && (
                      <div style={{
                        ...
                        <CheckCircle size={15} style={{ flexShrink: 0 }} /> {pendingAdminsNotice}
                      </div>
                    )}
                    {pendingAdminsError && (
                      <div style={{
                    ...
                    {pendingAdminsLoading ? (
                      <div style={{ padding: '40px 20px', textAlign: 'center', color: '#6a7e6c' }}>
            ...
            <div className="stat-grid">
              {statCards.map((item) => (
                <div key={item.key} style={{
                  ...
                  boxShadow: '0 6px 24px -12px rgba(15,36,21,0.10)',
                  ...
                  <div style={{ minWidth: 0 }}>
                    <p style={{ fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.15em', color: '#8a9e8c', margin: 0 }}>
                      {item.label}
                    </p>
                    <p style={{ fontSize: 26, fontWeight: 900, color: '#0e1e12', margin: '3px 0 2px', lineHeight: 1 }}>
                      {statValues[item.key]}
                    </p>
                    <p style={{ fontSize: 11, color: '#6c7b6e', margin: 0 }}>{item.note}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div style={{
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{
                    ...
                    <Database size={16} style={{ color: '#c0f0cc' }} />
                  </div>
                  <div>
                ...
                <div style={{
                  ...
                  color: '#8a4010', whiteSpace: 'nowrap',
                }}>
              ...
              {/* ── Desktop Table ── */}
              <div className="db-table" style={{ overflowX: 'auto', width: '100%' }}>
                    ...
                    <tr style={{ background: '#f2f8f2', borderBottom: '1px solid #d8e8da' }}>
                      {['#','Timestamp','Source','Client Name','Contact Info','Company','Country','Products','Context','Message / Details','Action'].map((h, i) => (
                        <th key={h} style={{
                          ...
                          textTransform: 'uppercase', color: '#6a8a6e', whiteSpace: 'nowrap',
                        }}>
                          {h}
                        </th>
                      ))}
                  ...
                  <tbody>
                    {currentRecords.length === 0 ? (
                      ...
                      currentRecords.map((sub, idx) => (
                        ...
                        >
                          <td style={{ padding: '11px 14px', borderRight: '1px solid #dce8de', textAlign: 'center', color: '#8aaa8e', fontFamily: "'DM Mono', monospace", fontSize: 12 }}>
                            {(currentPage - 1) * recordsPerPage + (idx + 1)}
                          </td>
                          <td style={{ padding: '11px 14px', borderRight: '1px solid #dce8de', whiteSpace: 'nowrap' }}>
                            <div style={{ fontWeight: 700, color: '#1e3822', fontSize: 13 }}>{new Date(sub.date).toLocaleDateString('en-GB')}</div>
                            <div style={{ fontSize: 10.5, color: '#8aaa8e', marginTop: 2 }}>{new Date(sub.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                          </td>
                          <td style={{ padding: '11px 14px', borderRight: '1px solid #dce8de' }}>
                            <span style={{ padding: '3px 10px', borderRadius: 99, background: '#e8f4ea', border: '1px solid #cce0d0', fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#2a6838' }}>
                              {sub.type || sub.tableName}
                            </span>
                          </td>
                          <td style={{ padding: '11px 14px', borderRight: '1px solid #dce8de', fontWeight: 700, color: '#0e1e12' }}>{sub.data.name || '-'}</td>
                          <td style={{ padding: '11px 14px', borderRight: '1px solid #dce8de' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                              {sub.data.email && (
                                <a href={`mailto:${sub.data.email}`} style={{ color: '#2a6838', textDecoration: 'none', fontSize: 12, display: 'inline-flex', alignItems: 'center', gap: 5 }}>
                                  <Mail size={11} /> {sub.data.email} <ExternalLink size={9} style={{ opacity: 0.5 }} />
                                </a>
                              )}
                              {sub.data.phone && (
                                <a href={`tel:${sub.data.phone}`} style={{ color: '#8a4010', textDecoration: 'none', fontSize: 12, fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: 5 }}>
                                  <Phone size={11} /> {sub.data.phone} <ExternalLink size={9} style={{ opacity: 0.5 }} />
                                </a>
                              )}
                            </div>
                          </td>
                          <td style={{ padding: '11px 14px', borderRight: '1px solid #dce8de', color: '#3a5040', fontWeight: 500 }}>{getCompanySection(sub.data)}</td>
                          <td style={{ padding: '11px 14px', borderRight: '1px solid #dce8de', color: '#3a5040' }}>{getCountrySection(sub.data)}</td>
                          <td style={{ padding: '11px 14px', borderRight: '1px solid #dce8de', color: '#3a5040' }}>{getProductsSection(sub.data)}</td>
                          <td style={{ padding: '11px 14px', borderRight: '1px solid #dce8de', color: '#3a5040' }}>{getContextSection(sub.data)}</td>
                          <td style={{ padding: '11px 14px', borderRight: '1px solid #dce8de', maxWidth: 240 }}>
                            <p style={{ margin: 0, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', color: '#4a6050', lineHeight: 1.5, fontSize: 12.5 }} title={getDetailSection(sub.data)}>
                              {getDetailSection(sub.data)}
                            </p>
                          </td>
                          <td style={{ padding: '11px 14px', textAlign: 'center' }}>
                            {sub.status === 'new' ? (
                              <button onClick={() => handleStatusChange(sub.id, 'reviewed', sub.tableName)} disabled={updatingId === sub.id} style={{
                                ...
                                {updatingId === sub.id ? <Loader2 size={11} className="animate-spin" /> : null}
                                PENDING
                              </button>
                            ) : (
                              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, color: '#1a6030', fontSize: 10, fontWeight: 800, letterSpacing: '0.06em' }}>
              ...
              {/* ── Mobile Cards (shown < 520px) ── */}
              <div className="mobile-cards">
                {currentRecords.length === 0 ? (
                  <div style={{ padding: '40px 20px', textAlign: 'center', color: '#8a9e8c', fontStyle: 'italic', fontSize: 14 }}>
                    No records found in database.
                  </div>
                ) : (
                  currentRecords.map((sub, idx) => (
                    <div key={`m-${sub.tableName}-${sub.id}`} style={{
                      background: '#fff', borderRadius: 16, border: '1.5px solid #dce8de',
                      padding: '16px', display: 'flex', flexDirection: 'column', gap: 10,
                    }}>
                      {/* Top row */}
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, color: '#8aaa8e' }}>
                            #{(currentPage - 1) * recordsPerPage + (idx + 1)}
                          </span>
                          <span style={{ padding: '2px 8px', borderRadius: 99, background: '#e8f4ea', border: '1px solid #cce0d0', fontSize: 10, fontWeight: 800, textTransform: 'uppercase', color: '#2a6838' }}>
                            {sub.type || sub.tableName}
                          </span>
                        </div>
                        <span style={{ fontSize: 11, color: '#8aaa8e' }}>
                          {new Date(sub.date).toLocaleDateString('en-GB')}
                        </span>
                      </div>
                      {/* Name */}
                      <p style={{ margin: 0, fontWeight: 700, fontSize: 15, color: '#0e1e12' }}>{sub.data.name || '-'}</p>
                      {/* Contact */}
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                        {sub.data.email && (
                          <a href={`mailto:${sub.data.email}`} style={{ color: '#2a6838', fontSize: 12, display: 'flex', alignItems: 'center', gap: 5, textDecoration: 'none' }}>
                            <Mail size={12} /> {sub.data.email}
                          </a>
                        )}
                        {sub.data.phone && (
                          <a href={`tel:${sub.data.phone}`} style={{ color: '#8a4010', fontSize: 12, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 5, textDecoration: 'none' }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                        {[
                          ['Company', getCompanySection(sub.data)],
                          ['Country', getCountrySection(sub.data)],
                          ['Products', getProductsSection(sub.data)],
                          ['Context', getContextSection(sub.data)],
                        ].map(([label, val]) => (
                          <div key={label}>
                            <p style={{ margin: 0, fontSize: 9, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#8a9e8c' }}>{label}</p>
                            <p style={{ margin: '3px 0 0', fontSize: 12, color: '#3a5040', fontWeight: 500 }}>{val}</p>
                          </div>
                        ))}
                      </div>
                      {/* Message */}
                      {getDetailSection(sub.data) !== '-' && (
                        <p style={{ margin: 0, fontSize: 12, color: '#4a6050', lineHeight: 1.5, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
                          {getDetailSection(sub.data)}
                        </p>
                      )}
                      {/* Action */}
                      <div>
                        {sub.status === 'new' ? (
                          <button onClick={() => handleStatusChange(sub.id, 'reviewed', sub.tableName)} disabled={updatingId === sub.id} style={{
                            ...
                            {updatingId === sub.id ? <Loader2 size={12} className="animate-spin" /> : null}
                            MARK PENDING
                          </button>
                        ) : (
                          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, color: '#1a6030', fontSize: 11, fontWeight: 800, letterSpacing: '0.05em' }}>
